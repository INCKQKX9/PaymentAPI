...
public class CreatePaymentAPIForMemberTestCases extends RestAssuredAPI {
	...
	public void tcAmwayAPIOnlineBankTransferCreatePayment(String refType, String currency, String amount,
		...
		try {
			...
			if (actualResultStatusCode == 200) {
				...
				Log.message("<i><b>   Validation for Get Payment API using payment ID" + "</b></i>");
				Thread.sleep(2000);
	...
	public void tcAmwayAPIAmpointsCreatePayment(String refType, String currency, String amount,
		...
		try {
			...
			if (callStatus == 200) {
				...
				Integer availablePoints = js.get("availablePoints");
				...
				if (callStatus == 200) {
					...
					int actualResultStatusCode = response.statusCode();
					Log.assertThat(actualResultStatusCode == 200,
							"Create Payment for Ampoints executed successfully as code is return as 200.",
							"Status Code is return as not 200 for create payment, Actual result : "
									+ actualResultStatusCode);
					String transStatus = createPaymentResponseDTO.getTransactions().get(0).getStatus();

					Log.assertThat(transStatus.equals("CAPTURED"),
							"transaction Status is CAPTURED. for create payment Ampoints",
							"Status is not CAPTURED for Ampoints, Actual result : " + transStatus);

					Log.assertThat(
							createPaymentResponseDTO.paymentId != null && !createPaymentResponseDTO.paymentId.isEmpty(),
							"Payment Id created successfully  for Ampoints",
							"Payment id has not generated for Ampoints.");
	...
	@Test(enabled = true, description = "Testcase to verify the create payment api for split", dataProviderClass = DataProviderUtils.class, dataProvider = "APIData")

	public void tcAmwayAPISplitAmpointOBTCreatePayment(String refType, String currency, String totalAmount,
			String accountId, String profileId, String obtPaymentMethodProviderId, String obtPaymentMethodCode,
			String obtEntryType, String obtAmount, String ampointPaymentMethodProviderId,
			String ampointPaymentMethodCode, String ampointEntryType, String ampointAmount) throws Exception {
		boolean result = false;
		...
		try {
			...
			String refId = String.valueOf(Utils.getRandom(1000, 100000));
			CreatePaymentUtility createPaymentUtilityObj = new CreatePaymentUtility();
			Response response = createPaymentUtilityObj.performSplitMemberCreatePayment(refId, refType, currency,
					Integer.parseInt(totalAmount), obtPaymentMethodProviderId, obtPaymentMethodCode, obtEntryType,
					ampointPaymentMethodProviderId, ampointPaymentMethodCode, ampointEntryType, accountId, profileId,
					Integer.parseInt(obtAmount), Integer.parseInt(ampointAmount));
			...
			if (actualResultStatusCode == 200) {
				...
				// once otp done check status is CAPTUERED OR NOT
				Log.message("<i><b>   Validation for Get Payment status using for split payment(Ampurse,OBT) "
						+ "</b></i>");
				Thread.sleep(2000);
				Response resPaymentData = createPaymentUtilityObj.performGetPaymentByPaymentId(paymentId);
				io.restassured.path.json.JsonPath paymentJs = createPaymentUtilityObj.getJsonPathObject(resPaymentData);
